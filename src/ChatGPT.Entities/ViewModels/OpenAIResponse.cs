...
public class OpenAIResponse
{
    ...
    /// Constructor
    /// </summary>
    public OpenAIResponse(OpenAI_API.Chat.ChatResult okResult)
    {
        var newChoices = new List<OpenAIResponseChoice>();
        foreach (var item in okResult.Choices)
        {
            newChoices.Add(new OpenAIResponseChoice(item.Message.Content) { FinishReason = item.FinishReason });
        }
        Choices = newChoices.ToArray();
        Usage = new OpenAIResponseUsage(okResult.Usage.CompletionTokens, okResult.Usage.PromptTokens, okResult.Usage.TotalTokens);
        Model = okResult.Model.ModelID;
    }
}
...
public class OpenAIResponseUsage
{
    ...
    /// Constructor
    /// </summary>
    public OpenAIResponseUsage(int completionTokens, int promptTokens, long totalTokens)
    {
